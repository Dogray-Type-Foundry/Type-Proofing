...
def get_font_info(font_path):
    ...
    # Extract per-axis named instance values and fill in any axes missed by DrawBot
    try:
        f = get_ttfont(font_path)
        if f and "fvar" in f:
            fvar = f["fvar"]
            # Ensure all fvar axes are present in font_info["axes"]
            for a in fvar.axes:
                tag = a.axisTag
                if tag not in font_info["axes"]:
                    mn = float(a.minValue)
                    df = float(a.defaultValue)
                    mx = float(a.maxValue)
                    vals = [int(v) if v == int(v) else v for v in [mn, df, mx]]
                    ...
                    for v in vals:
                        if v not in seen:
                            seen.add(v)
                            unique.append(v)
                    font_info["axes"][tag] = unique

            axis_tags = [a.axisTag for a in fvar.axes]
            # Collect all instance coordinate values per axis
            per_axis = {tag: set() for tag in axis_tags}
            for inst in fvar.instances:
                for tag, val in inst.coordinates.items():
                    if tag in per_axis:
                        per_axis[tag].add(float(val))
            font_info["axis_instances"] = {
                tag: sorted(vals) for tag, vals in per_axis.items() if vals
            }
    except Exception:
        pass
