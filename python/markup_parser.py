...
def _tokenize(raw_text):
    ...
    for i, line in enumerate(lines):
        ...
        if stripped == "":
            ...
            tokens.append(Token(kind="page_break"))
        elif stripped == "#colbreak()":
            tokens.append(Token(kind="column_break"))
...
def _resolve_font_by_name(name, all_fonts):
    """Resolve a font style name to a (font_path, coordinates_or_None) tuple.

    Checks in order: SubFamilyName, PostScript name, VF named instance names.
    Returns (font_path, None) for static/font-level matches, or
    (font_path, coordinates_dict) for VF named instance matches, or
    (None, None) if not found.
    """
    name_cleaned = name.strip().strip('"').strip("'")
    name_lower = name_cleaned.lower()
    # Remove spaces for PS-name style matching
    name_nospace = name_cleaned.replace(" ", "").lower()
    ...
    for font_path in all_fonts:
        ...
        # 1. Check SubFamilyName
        subfamily = tt["name"].getBestSubFamilyName()
        if subfamily and subfamily.lower() == name_lower:
            return font_path, None

        # 2. Check PostScript name
        try:
            ps_name = db.font(font_path)
            if ps_name:
                ps_lower = ps_name.lower()
                if ps_lower == name_lower or ps_lower == name_nospace:
                    return font_path, None
                if ps_lower.endswith(f"-{name_nospace}"):
                    return font_path, None
        except Exception:
            pass

        # 3. Check VF named instances
        if "fvar" in tt:
            name_table = tt["name"]
            family_name = name_table.getBestFamilyName() or ""
            family_lower = family_name.lower()
            family_nospace = family_name.replace(" ", "").lower()

            for inst in tt["fvar"].instances:
                inst_name_record = name_table.getName(
                    inst.subfamilyNameID, 3, 1, 0x0409
                )
                if inst_name_record is None:
                    continue
                inst_name = str(inst_name_record)
                inst_lower = inst_name.lower()
                inst_nospace = inst_name.replace(" ", "").lower()
                coords = {k: float(v) for k, v in inst.coordinates.items()}

                # Match against: instance name alone, family-instance, PS-style
                if inst_lower == name_lower or inst_nospace == name_nospace:
                    return font_path, coords
                # "FamilyName-InstanceName" (PS-name style)
                constructed_ps = f"{family_nospace}-{inst_nospace}"
                if constructed_ps == name_nospace:
                    return font_path, coords
                # With space: "Family Name Instance Name"
                constructed_full = f"{family_lower} {inst_lower}"
                if constructed_full == name_lower:
                    return font_path, coords
...
def _build_attr_overrides(attrs, base_font, all_fonts, base_axis_dict, base_otfeatures):
    ...
    for key, value in attrs.items():
        if key in ("wght", "opsz", "wdth"):
            ...
            font_path, style_coords = _resolve_font_by_name(value, all_fonts)
            if font_path:
                overrides["font"] = font_path
                if style_coords:
                    # VF named instance: apply its coordinates
                    variations.update(style_coords)
                    has_variations = True
        elif key == "feat":
...
def _build_formatted_string(
    ...
    if base_line_height_ratio:
        base_kwargs["lineHeight"] = base_line_height_ratio * base_font_size
    ...
    for token in tokens:
        try:
            ...
            if token.kind == "column_break":
                current_page.append(fs)
                fs = db.FormattedString(txt="", **base_kwargs)
                skip_next_newline = True
                continue

            if skip_next_newline and token.kind == "plain" and token.text == "\n":
                skip_next_newline = False
                continue
            skip_next_newline = False
            ...
            if token.kind == "plain":
                ...
                heading_size = min(base_font_size * multiplier, 90)
                h_kwargs["fontSize"] = heading_size
                if base_line_height_ratio:
                    h_kwargs["lineHeight"] = base_line_height_ratio * heading_size
                ...
                # Recompute lineHeight for custom font size
                if base_line_height_ratio and "fontSize" in a_kwargs:
                    a_kwargs["lineHeight"] = (
                        base_line_height_ratio * a_kwargs["fontSize"]
                    )
...
def parse_custom_text(
    ...
    """Parse markdown-like text and return nested lists of styled FormattedStrings.

    Page breaks (``#pagebreak()``) split text into separate page segments.
    Column breaks (``#colbreak()``) split each page into column segments.

    ``base_line_height_ratio`` is a unitless em multiplier (like CSS
    ``line-height: 1.5``).  Each text run computes its own absolute
    ``lineHeight`` as ``ratio × that_run’s_fontSize``.

    Returns ``List[List[FormattedString]]`` — outer list is pages, inner
    list is column segments within each page.
    """
    try:
        ...
        return _build_formatted_string(
            ...
            base_otfeatures,
            base_axis_dict,
            base_line_height_ratio=base_line_height_ratio,
        )
    except Exception as e:
        ...
        if base_line_height_ratio:
            kwargs["lineHeight"] = base_line_height_ratio * base_font_size
