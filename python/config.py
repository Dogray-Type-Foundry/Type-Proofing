...
FALLBACK_FONT = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "AdobeBlank.otf"
)
...
def setup_page_format(page_format):
    ...
    Updates the global pageDimensions variable used by proof generation.
    """
    ...
    if isinstance(page_format, (list, tuple)) and len(page_format) >= 2:
        ...
        print(f"Invalid page format: {page_format}")
        return False
